...
import './TopRatedMovieDetail.css';
...
const TopRatedMovieDetail = () => {
  ...
  useEffect(() => {
    const getMovieData = async () => {
      try {
        const movieRef = doc(db, "top-rated", id); // Changed to "top-rated"
  ...
  // Fetch Comments
  useEffect(() => {
    const q = query(collection(db, "top-rated", id, "comments")); // Changed to "top-rated"
  ...
  const handleAddComment = async () => {
    ...
    try {
      await addDoc(collection(db, "top-rated", id, "comments"), { // Changed to "top-rated"
        text: comment,
